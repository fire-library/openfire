...
use ::openfire::cibse_guide_e::chapter_6::appendix::{
    ...
    common as rust_common, d_compartment_with_core as rust_d_compartment_with_core,
};
...
fn internal_surface_area_a(a_f: f64, h: f64, w1: f64, w2: f64, a_o: f64) -> PyResult<f64> {
    Ok(rust_a_simple_case::internal_surface_area(a_f, h, w1, w2, a_o))
}
...
fn areas_of_openings_multiple_openings(openings_dimensions: Vec<(f64, f64)>) -> PyResult<Vec<f64>> {
    Ok(rust_common::areas_of_openings_multiple_openings(openings_dimensions))
}
...
#[pyfunction]
fn areas_of_openings_multiple_openings_b(openings_dimensions: Vec<(f64, f64)>) -> PyResult<Vec<f64>> {
...
fn sum_areas_of_openings_b(areas_of_openings: Vec<f64>) -> PyResult<f64> {
    Ok(rust_b_more_than_one_window::sum_areas_of_openings(areas_of_openings))
}
...
fn equivalent_height_for_compartment_openings_b(
    ...
    Ok(rust_b_more_than_one_window::equivalent_height_for_compartment_openings(equivalent_area_of_openings, equivalent_width_of_openings))
}
...
fn b_more_than_one_window(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(equivalent_height_for_compartment_openings_b, m)?)?;
    Ok(())
}
...
#[pyfunction]
fn sum_area_of_openings_per_wall(dimensions_of_openings_wall_per_wall: Vec<(f64, f64)>) -> PyResult<f64> {
    Ok(rust_c_windows_in_more_than_one_wall::sum_area_of_openings_per_wall(dimensions_of_openings_wall_per_wall))
}

#[pyfunction]
fn sum_area_of_openigs(areas_of_openings: Vec<f64>) -> PyResult<f64> {
    Ok(rust_c_windows_in_more_than_one_wall::sum_area_of_openigs(areas_of_openings))
}
...
fn internal_surface_area_of_compartment_with_core(
    ...
    Ok(rust_d_compartment_with_core::internal_surface_area_of_compartment_with_core(w1, w2, h, c1, c2, ch, a_o))
}
...
fn ratio_depth_over_height_compartment_with_core(
    ...
    Ok(rust_d_compartment_with_core::ratio_depth_over_height_compartment_with_core(w1, w2, c1, c2, ao_w1, ao))
}
...
fn d_compartment_with_core(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(internal_surface_area_of_compartment_with_core, m)?)?;
    m.add_function(wrap_pyfunction!(ratio_depth_over_height_compartment_with_core, m)?)?;
    Ok(())
}
