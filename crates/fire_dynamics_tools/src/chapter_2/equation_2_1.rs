pub fn hot_gas_temperature_natural_ventilation_mqh(q: Vec<f64>, a_v: Vec<f64>, h_v: Vec<f64>, a_t: f64, h_k: f64) -> Vec<f64> {
    let area_av_times_hv: f64 = a_v.iter().zip(h_v.iter()).map(|(av, hv)| av * hv.powf(0.5)).sum();
...
pub fn hot_gas_temperature_natural_ventilation_mqh_equation(delta_t_g: String, q: String, a_v:String, h_v: String, a_t: String, h_k: String) -> String {
    format!("{} = 6.85 \\cdot \\left( \\frac{{{}^{{2}}}}{{\\left(\\sum_{{i}} {} \\cdot \\sqrt{{{}}}\\right) \\cdot ({} \\cdot {})}} \\right)^{{1/3}}", delta_t_g, q, a_v, h_v, a_t, h_k)
}
